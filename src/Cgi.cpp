...
void Cgi::run(std::map<std::string, std::string> requestHeader, int client_fd)
{
	...
	if(!is_python3_installed()){
		std::cerr << "Python not installed\n";
		throw(CgiException());
	}
	if(!is_python_file(requestHeader["location:"])){
		std::cerr << "File is not Python\n";
		throw(CgiException());
	}
	int save_stdin = dup(STDIN_FILENO);
	int save_stdout = dup(STDOUT_FILENO);
	std::cout << "cgi: request header method [" << requestHeader["method:"] << "]\n";
	if (!is_query_string(requestHeader))
	{
		// body_path = create_request_body_file(requestHeader, client_fd);
		std::cout << "cgi: the body path is " << body_path << "\n";
		// const char* in_filename = body_path.c_str();
		std::string filename = "../HTML/cgi-bin/in_file";
		const char* in_filename;
		in_filename = filename.c_str();
		infile = open(in_filename, O_RDONLY);
		if (infile == -1) {
			std::cerr << "cgi: Error opening the input file.\n";
			throw(CgiException());
    	}
		// std::cout << "The file contents are as follows: \n";
		// printFileContents(in_filename);
		// exit(0);
	}
	...
	if (_cgi_pid == 0)
	{
		...
		std::string path_adder = "../HTML" + requestHeader["location:"];
		...
		if(!is_query_string(requestHeader)){
			env_variable = "CONTENT_LENGTH=" + requestHeader["Content-Length:"];
			enviromentals.push_back(env_variable);
		}

		// CONTENT_TYPE: "text/html" represents the media type of the request body
		if(!is_query_string(requestHeader)){
			env_variable = "CONTENT_TYPE=" + requestHeader["Content-Type:"];
			enviromentals.push_back(env_variable);
		...
		if (is_query_string(requestHeader)) {
			env_variable = "QUERY_STRING=" + requestHeader["query_string:"];
			enviromentals.push_back(env_variable);	
		}

		// REQUEST_METHOD: "POST" (indicates the HTTP request method)
		env_variable = "REQUEST_METHOD=" + requestHeader["method:"];
		...
		env_variable = "SCRIPT_NAME=" + requestHeader["location:"];
    	enviromentals.push_back(env_variable);
		...
		env_variable = "SERVER_PROTOCOL=" + requestHeader["HTTP_version:"];
    	enviromentals.push_back(env_variable);
...
std::string	Cgi::create_request_body_file(std::map<std::string, std::string> requestHeader, int clientFd)
{
    char buf[BUF_SIZE];
	...
	filename << "cgi_input_file.tmp";
	std::ofstream outFile(filename.str(), std::ios::binary | std::ios::trunc);

	size_t	request_body_size = atol(requestHeader["Content-Length:"].c_str());
	...
	while (remaining_bytes)
	{
    	...
    	int read_bytes;
    	read_bytes = recv(clientFd, buf, request_body_size, MSG_DONTWAIT);
		if (read_bytes <= 0)
			break;
    ...
    recv(clientFd, buf, BUF_SIZE, MSG_DONTWAIT);
	return (filename.str());
}
...
bool Cgi::is_query_string(std::map<std::string, std::string> requestHeader) {
    std::string key_to_find = "query_string:";
    std::map<std::string, std::string>::iterator it = requestHeader.find(key_to_find);
    return (it != requestHeader.end()) ? true : false;
}
